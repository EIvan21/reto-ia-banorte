...
# --- El contenedor de las herramientas de servidor --------------------------
# Pegar la URL de una vacante fallaba SIEMPRE con 400: "container_id is required
# when there are pending tool uses generated by code execution with tools".
# web_fetch arrastra a code_execution, esas corren en un contenedor remoto, y
# las llamadas siguientes del mismo turno tienen que referenciarlo.


def test_el_contenedor_viaja_en_la_peticion_cuando_existe():
    from app.agent import _construir_kwargs

    sin = _construir_kwargs([{"role": "user", "content": "hola"}], [])
    assert "container" not in sin, "no debe mandarse un contenedor vacio"

    con = _construir_kwargs([{"role": "user", "content": "hola"}], [], contenedor="container_abc")
    assert con["container"] == "container_abc"


def test_el_loop_captura_el_contenedor_de_la_respuesta():
    """Si alguien quita esto, pegar un enlace vuelve a fallar con 400 y la
    prueba de arriba sigue pasando: hay que verificar que se LEE, no solo que
    se manda."""
    import inspect
    ...
    assert 'getattr(mensaje, "container"' in fuente, "no se lee el contenedor de la respuesta"
    assert "effort, contenedor" in fuente, "el contenedor no se propaga a la siguiente llamada"


def test_las_senales_factuales_no_marcan_un_saludo():
    """El agente se presenta nombrando Looker, LookML y BigQuery, que vienen de
    su linea de identidad en el prompt y no de una herramienta. Tenerlas como
    senal hacia que cada saludo saliera etiquetado, y un falso positivo en una
    alerta entrena a ignorarla."""
    saludo = (
        "Hola! Soy el agente del CV de Edher, ingeniero que construye agentes de IA "
        "sobre Google Cloud, con base en Analytics Engineering (Looker, LookML, BigQuery)."
    )
    assert guardrails.verificar_fundamento(saludo, []) == []


@pytest.mark.parametrize("texto", [
    "Edher trabajo en GlobalLogic y redujo 30% el tiempo de desarrollo.",
    "Esta certificado como Associate Cloud Engineer.",
    "Se graduo con la Medalla al Merito Universitario.",
])
